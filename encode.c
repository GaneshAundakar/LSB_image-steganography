...
/* Get image size, Input: Image file ptr Output: width * height * bytes per pixel (3 in our case)
...
/* Get File pointers for i/p and o/p files Inputs: Src Image file, Secret file and Stego Image file Output: FILE pointer for above files
...
Status read_and_validate_encode_args(char *argv[], EncodeInfo *encInfo)
{
    ...
    {
        printf("Receieved a proper bmp file \n");
        encInfo -> src_image_fname = argv[2];
    }
    else
    {
        printf("Failed to recieve bmp file\n");
        return e_failure;
    }

    if( strcmp(strchr(argv[3], '.'), ".txt") == 0)
    {
        printf("Receieved a proper text file \n");
        encInfo -> secret_fname = argv[3];
    }
    else
    {
        printf("Failed to recieve txt file\n");
        return e_failure;
    }

    if(argv[4] != NULL)
    {
        printf("Recieved the output file name \n");
        encInfo -> stego_image_fname = argv[4];
    }
    else
    {
        printf("Haven't recieved the output file name. hence creating it as stego.bmp\n ");
...
uint get_file_size(FILE *fptr)
{
    ...
    fseek(fptr, 0, SEEK_SET); // Reset pointer
    return size;
}
...
Status do_encoding(EncodeInfo *encInfo)
{
    ...
    {
        printf("\nInfo : Opened required files in required mode successfully\n");
    }
    else
    {
    ...
    if(check_capacity (encInfo) == e_success)
        printf("\nInfo : Yes. %s file is suitable for encoding\n", encInfo -> src_image_fname);
    else
        printf("No. %s file is suitable for encoding\n", encInfo -> src_image_fname);

    printf("\nInfo : Copying the header from %s to %s as it is. \n", encInfo -> src_image_fname, encInfo -> stego_image_fname);
    ...
    printf("\nInfo : Started to encode magic string\n");

    if(encode_magic_string(MAGIC_STRING, encInfo) == e_success)
    {
        printf("\t ---> Done. completed encoding the magic string\n");
    }
    else
    {
    ...
    if(encode_extension_size (4, encInfo) == e_success)
        printf("\nInfo : Encoded secret file extension size successfully\n");
    else
    {
    ...
    if(encode_secret_file_extn (encInfo -> extn_secret_file, encInfo) == e_success)
        printf("\nInfo : Encoded secret file successfully\n");
    else
    {
